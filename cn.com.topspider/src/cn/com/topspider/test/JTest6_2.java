...
public class JTest6_2 {
	public static void main(String[] args) throws IOException {
		...
		String tieziUrl = "https://tieba.baidu.com/p/8314016158"; //帖子url
...
//				.cookies(map)
		...
		for (Element postElmt : listSelect) {
			...
			String attr1 = postElmt.attr("data-pid");
			if(StringUtils.isBlank(attr1)){ //广告
				continue;
			}
			...
			if(StringUtils.isNotBlank(tailSelect.text())){
...
//			System.out.println(select.text());
			...
			//当data_field为空时取下面属性
			if(StringUtils.isBlank(data_field)){
				String attr = postElmt.attr("data-field");
				JSONObject content = JSONObject.parseObject(unicodeToString(attr)).getJSONObject("content");
				Long post_id = content.getLong("post_id"); //楼层pid
				Integer comment_num = content.getInteger("comment_num"); //回复数量
				Integer post_no = content.getInteger("post_no"); //楼层
				String ip_address = content.getString("ip_address"); //ip属地
				String date = content.getString("date"); //日期
				pid = post_id;
				total_num = comment_num;
				if(StringUtils.isBlank(tailSelect.text())){
					System.out.println("IP属地:" + ip_address +"," + post_no + "楼" + date +",回复数:" + comment_num);
				}
			}else{
				pid = JSONObject.parseObject(data_field).getLong("pid");
				total_num = JSONObject.parseObject(data_field).getInteger("total_num");
			}
			if(total_num != null && total_num > 0){
//				pid = (Long)JSONObject.parseObject(data_field).get("pid");
//				total_num = (Integer) JSONObject.parseObject(data_field).get("total_num");
				if(total_num != null && total_num != 0) {
